...
socket.on('chat_response', function(data) {
    ...
    // Check if the message contains a 'finish_reason' property
    if (data.hasOwnProperty('finish_reason')) {
      if (data.finish_reason === 'stop') {
        // If the 'finish_reason' is 'stop', it indicates the end of the message
        $('.ai:last .formatted-message').data('partial', false);
      }
    } else {
      // If there is no 'finish_reason' property, assume the message is incomplete
      $('.ai:last .formatted-message').data('partial', true);
    }
  
    // Check if a partial message already exists in the chatroom
    if ($('.ai:last .formatted-message').data('partial')) {
      // Update the last AI message bubble with the new content
      var currentContent = $('.ai:last .formatted-message').html();
      $('.ai:last .formatted-message').html(sanitizeHTML(currentContent + data.message));
    } else {
      // Create a new message bubble with a 'data-partial' attribute set to true
      add_message_to_chat('ai', data.message, /* isPartial */ true);
    }
